...
namespace MixERP.Purchases.DAL.Backend.Tasks
{
    public static class Orders
    {
        public static async Task<IEnumerable<dynamic>> GetSearchViewAsync(string tenant, int officeId, OrderSearch search)
        {
            ...
            {
                var sql = new Sql("SELECT * FROM purchase.order_search_view");
                ...
                sql.And("order_id::text LIKE @0", search.Id.ToSqlLikeExpression().ToLower());
        ...
        public static async Task CancelAsync(string tenant, long id, LoginView meta)
        {
            using (var db = DbProvider.Get(FrapidDbServer.GetConnectionString(tenant), tenant).GetDatabase())
            {
                var sql = new Sql("UPDATE purchase.orders");
                sql.Append("SET");
